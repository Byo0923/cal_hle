...
#SBATCH --nodelist=osk-gpu69,osk-gpu70,osk-gpu71
#SBATCH --nodes=3
...
# ====== 固定公開先（ポートは固定、IPはヘッドに追随） ======
SERVE_PORT=8010
SERVE_HOST="${SERVE_HOST:-$head_node_ip}"   # ← 自動でヘッドのIPに
echo "[INFO] Reserved serve endpoint → ${SERVE_HOST}:${SERVE_PORT}"

# （厳密固定したい場合だけ有効化）
# if [[ -n "${SERVE_HOST_FORCE:-}" && "$head_node_ip" != "$SERVE_HOST" ]]; then
#   echo "[ERROR] Head IP ($head_node_ip) != SERVE_HOST ($SERVE_HOST)."; exit 3
# fi

# ポート衝突チェック（ヘッドで）
srun -N1 -n1 -w "$head_node" bash -lc \
  "ss -H -ltn | awk -v p=\":$SERVE_PORT\" '\$4 ~ p {f=1} END{exit !f}'" \
  && { echo "[ERROR] Port $SERVE_PORT is already in use on $SERVE_HOST."; exit 2; }
...
echo "[INFO] Launching vLLM on ${SERVE_HOST}:${SERVE_PORT}  ..."

export VLLM_HOST_IP=192.168.1.69
export RAY_ADDRESS=192.168.1.69:37173
...
MODEL_DIR=/home/Competition2025/P07/shareP07/K_Nishizawa/deepseek-prover-671b_awq/bf16_deepseek-prover-v3-671b
SERVED_NAME="${SERVED_NAME:-deepseek-prover-671b}"
export MODEL_DIR

python - <<'PY'
import json,sys,os
p=os.environ["MODEL_DIR"]+"/config.json"
...
$VLLM serve "$MODEL_DIR" \
  ...
  --dtype bfloat16 \
  --host 192.168.1.69 --port 8010 \
  --served-model-name deepseek-prover-671b \
  --max-model-len 4096 \
  --trust-remote-code \
  --enforce-eager
